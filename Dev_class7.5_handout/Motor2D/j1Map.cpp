...
void j1Map::PropagateBFS()
{
	...
	if (frontier.Count() != NULL) {
		...
		frontier.Pop(coords);
		iPoint neighbors[4];
		neighbors[0] = { coords.x ,coords.y - 1 };
		neighbors[1] = { coords.x + 1,coords.y };
		neighbors[2] = { coords.x,coords.y + 1 };
		neighbors[3] = { coords.x - 1,coords.y };


		for (int i = 0; i < 4; i++) {
			if (visited.find(neighbors[i]) == -1 && IsWalkable(neighbors[i].x, neighbors[i].y)) {
...
bool j1Map::IsWalkable(int x, int y) const
{
	...
	return (item->data->Get(x, y) == 0 && x >= 0 && x < data.width && y >= 0 && y < data.height);
}
