...
// then call all the modules to load themselves
	...
	pugi::xml_node		loadgame;
	pugi::xml_parse_result result = loadgame_file.load_file("savegame.xml");
	bool ret=false;
	
	if (result == true)
	{
		loadgame = loadgame_file.child("save");
		p2List_item<j1Module*>* item;
		item = modules.start;
		ret = true;
		while (item != NULL && ret == true)
		{
			ret = item->data->Load(loadgame_file.child(item->data->name.GetString()));
			item = item->next;
		}
	}
...
// TODO 7: Create a method to save the current state
	...
	bool ret = true;
	savegame = savegame_file.append_child("save");
		...
		while (item != NULL && ret == true)
		{
			ret = item->data->Save(savegame_file.append_child(item->data->name.GetString()));
			item = item->next;
		}
	...
	void j1App::LoadButtonPressed() {
		loadbuttonpressed = true;
	}
	void j1App::SaveButtonPressed() {
