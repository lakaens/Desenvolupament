...
void j1Map::Draw()
{
	...
	for (uint x = 0; x < data.width; x++) {
		for (uint y = 0; y < data.height; y++) {
			...
			App->render->Blit(tileset->texture, coordinates.x, coordinates.y, &rect);
		}
	}
...
iPoint j1Map::WorldToMap(int x, int y) const
{
	...
	if (data.type == MapTypes::MAPTYPE_ORTHOGONAL) {
		x = ret.x/data.tile_width;
		y = ret.y/data.tile_height;
	}
	if (data.type == MapTypes::MAPTYPE_ISOMETRIC) {
		/*ret.x = ((x / data.tile_width*0.5f) + y);
		ret.y = ((y / data.tile_height*0.5f) - x);*/
		//int x1, y1;
		//x1 = ret.x / data.tile_width*0.5f;
		//y1 = ret.y / data.tile_height*0.5f;
		////x = (x1 + y1) / 2;
		//y = (x1 - y1)/-2;
		//x = x1 + y;
		...
		int x1, y1;
		x1 = x / data.tile_width*0.5f;
		y1 = y / data.tile_height*0.5f;
		/*ret.x = (x1 + y1) / 2;
		ret.y = ret.x - x1;*/
		ret.y = (x1 - y1) / 2;
		ret.x = x1 + y;
